...
interface ClassificationRule {
  ...
  readonly kind: FailureKind;
  readonly confidence: Confidence;
  /** Return the matched signals, or null for no match. */
  match(f: ClassifiableFailure): string[] | null;
}

const has = (haystack: string, needle: string): boolean =>
  haystack.toLowerCase().includes(needle.toLowerCase());

const anyOf = (haystack: string, needles: readonly string[]): string[] =>
  needles.filter(n => has(haystack, n));

const matcherIs = (f: ClassifiableFailure, ...names: readonly string[]): boolean =>
  f.matcher !== null && names.some(n => f.matcher?.toLowerCase().includes(n.toLowerCase()) === true);
...
const RULES: readonly ClassificationRule[] = [
  ...
  },
  {
    ...
    match: f => {
      ...
      const consoleHits = f.consoleErrors.filter(e =>
        UNCAUGHT_PATTERNS.some(p => has(e, p)),
      );
  ...
  },
  {
    ...
    confidence: 'high',
    match: f => {
      if (matcherIs(f, 'toBeOK')) return ['matcher toBeOK failed'];
      const hits = anyOf(f.message, [
        'Unexpected status',
        'expected status',
        'toBeOK',
  ...
  // ── 4. Snapshot comparisons ──────────────────────────────────────────────
  {
    ...
    confidence: 'high',
    match: f => {
      if (matcherIs(f, 'toHaveScreenshot')) return ['matcher toHaveScreenshot'];
      const hits = anyOf(f.message, [
  ...
  },
  {
    ...
    confidence: 'high',
    match: f => {
      if (matcherIs(f, 'toMatchAriaSnapshot')) return ['matcher toMatchAriaSnapshot'];
  ...
  },
  {
    ...
    match: f => {
      ...
      const actionTimeout =
        f.timedOut && /locator\.\w+:|frameLocator|\.click\(|\.fill\(/.test(f.message)
  ...
  },
  {
    ...
    match: f => {
      ...
      const hasExpectedReceived =
        (has(f.message, 'Expected') && has(f.message, 'Received')) ||
...
const TEST_TIMEOUT_RULE: ClassificationRule = {
  name: 'timeout.test-level',
  kind: 'assertion_visibility',
  confidence: 'low',
  match: f =>
    f.timedOut || f.budgetUsedRatio >= 0.95
