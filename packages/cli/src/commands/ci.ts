/**
 * `atest ci generate` — emit a workflow for a consumer repository.
 *
  ...
  policy:
    ...
    steps:
      ...
      - name: Quarantine hygiene
        run: npx atest flaky expire --ci
`;
}

function gitlabWorkflow(options: TemplateOptions): string {
  return `# Generated by \`atest ci generate --provider gitlab\`.
#
...
analyze:
  ...
  script:
    ...
    - npx atest flaky report --runs .atest/runs
    - npx atest heal --evidence .atest/evidence --dry-run || true
    - npx atest report --runs .atest/runs --evidence .atest/evidence --out report.html
  artifacts:
    when: always
    paths:
      - report.html
