 ...
 */
export function previewValue(value: unknown, depth = 0): string {
...
export function previewArgs(args: readonly unknown[]): string {
 ...
 */
export function bindPage<T extends object>(mod: T, page: Page, name?: string): BoundPageObject<T> {
  ...
  for (const [key, value] of Object.entries(mod)) {
    ...
    const fn = value as (...rest: unknown[]) => unknown;
    ...
    if (!isAsyncFunction(fn)) {
      bound[key] = (...args: unknown[]) => fn(page, ...args);
      continue;
    }

    const label = name === undefined ? key : `${name}.${key}`;
    bound[key] = (...args: unknown[]) =>
      test.step(`${label}(${previewArgs(args)})`, () => fn(page, ...args) as Promise<unknown>);
  }
