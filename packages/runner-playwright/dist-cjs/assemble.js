...
exports.toClassifiable = toClassifiable;
...
function joinErrors(errors) {
    return {
        message: (0, errors_js_1.stripAnsi)(errors.map(e => e.message ?? '').filter(Boolean).join('\n\n')),
        stack: (0, errors_js_1.stripAnsi)(errors.map(e => e.stack ?? '').filter(Boolean).join('\n\n')),
    };
}
...
function seedCandidates(intended, present) {
    const parsed = (0, core_1.parseLocator)(intended);
    if (parsed === null || parsed.strategy !== 'testid')
        return [];
    return present
        .filter(id => id !== parsed.value)
        .map(id => ({ id, distance: (0, core_1.testIdDistance)(parsed.value, id) }))
...
function toClassifiable(result, sidecars, timeoutMs) {
    const { message, stack } = joinErrors(result.errors);
    const parsed = (0, errors_js_1.parsePlaywrightError)(message, stack);
    return {
        message,
        stack,
...
function assembleBundle(input) {
    ...
    const classification = (0, core_1.classify)(classifiable);
    const steps = (0, steps_js_1.extractSteps)(result.steps);
    const failingStep = (0, steps_js_1.findFailingStep)(result.steps);
...
function classifyResult(result, sidecars, timeoutMs) {
    return (0, core_1.classify)(toClassifiable(result, sidecars, timeoutMs));
}
